...
cbPalette <- c("#009E73", "#56B4E9", "#E69F00", "#F0E442", "#0072B2", "#D55E00", "#CC79A7") 
theme_set(theme_bw())
...
spk_bar <- exp_data %>%
  group_by(coding) %>%
  filter(coding == 246 | coding == 340 | coding == 723) %>% 
  summarise( 
    n=n(),
...
bar_spk <- ggplot(spk_bar, aes(y = mean)) +
  geom_bar(aes(x = coding, fill = coding), stat="identity", alpha=0.7) +
  geom_errorbar(aes(x = coding, 
                    ymin=mean-ic, 
