...
### DATA WRANGLING
## read messy dataset
df.sibilant <- read_csv("~/masc_perception_2spk/data/masc_2pk_pilot_v2-merged.csv")
df.survey <- read_csv("~/masc_perception_2spk/data/survey_pilot_v2.csv")
...
df.numeric$sib_code <- factor(df.numeric$sib_code, 
                              levels = c("control", 
                                         ...
                                         "mid", 
                                         "fronted"))
df.numeric$response <- as.double(df.numeric$response)/100
...
fit.lmer <- lmer(response ~ sib_code * spk + 
                   (1 | workerid) + 
                   (1 | triplet_id), 
                 data = df.numeric)
