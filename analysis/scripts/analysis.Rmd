...
fit.lmer <- lmer(response ~ gender_corr*spk*srq_scoreC +
                   sib_code +
                   spk +
                   prop_creakC + 
                   speech_rateC + 
                   ...
                   # f2_raw*sib_code + # exclude bc using f2 removes control condition
                   (1 | workerid) + 
                   (1 | triplet_id), 
                 data = df.numeric,
                 REML = F)
...
# LOAD fit zoib model 
#fit.zoib <- readRDS("~/masc_perception_2spk/brm_zoib_3rd.rds")
...
fit.zoib %>% 
  ...
  labs(title = "Pairwise comparisons of /s/ condition by speaker",
       y = "speaker")
...
df.phon.test1 <- rbind(data.frame(Hypothesis = "Creak Effect?", 
                                  phon.h1$hypothesis)) %>% 
  select(-Hypothesis.1)
